...
**Configure profile (Options).** Under **Options → JSON profile**, **Configure profile** opens a tabbed dialog for pack chrome that used to require unzipping the ZIP: nav **logo** (two words + optional hex colours), header **identity** (usernames + hostname), Workspace **texts** (buttons, placeholders, messages), **project** URLs, and **synopsis**. Saving writes the live session (and updates the active personal pack snapshot when one is selected). Theme, language, LLM, voice dictation, and Prompts still change **in the live UI** before export — an info panel in the dialog explains that path.

**One UI language in the app.** Configure profile (and the live session before export) only covers the **current UI display language**. PromptDeMerde does **not** ship an in-app editor to fill every other pack language one by one. For those locales — extra prompt Markdown, maximal embedded UI dictionaries, per-locale contexts — export the ZIP, **unzip**, edit the JSON / Markdown files, **re-zip**, then import again. That hand-edit path is the supported way today; a richer multi-locale editor may appear later as a **Marketplace extension** once the catalogue can host UI extensions. Until then, treat the single-language Configure path as the product default, not a missing screen.
...
**How to customise deliberately.** Prefer **Configure profile** for brand / identity / texts / synopsis **in the active UI language**. For the rest of the live pack (theme, LLM, Prompts…), set Options / Workspace / Prompts, then export (**5.5.1**). For **other languages** inside the archive, use the power path: export (often **maximal**), unzip, edit JSON/Markdown, re-zip, import again. Validation stays in the browser; the server never reads the archive contents. Keep a copy of the previous ZIP before re-import if the current session must be recoverable — import replaces the local session.
...
Those packs appear in **Options → JSON profile** after an event-driven rescan (boot, opening Options, or returning to the tab), with one exception: if the same pack is already present as a **native** bundle under `assets/profiles/` (badge `(native)` — shipped in the repo and still on disk for that install), the matching free ZIP is **not** listed again. The ZIP file stays available for download or **Import** when a clean archive is needed. During rescan each free ZIP is validated in order; invalid archives are skipped and never appear in the selector. Selector order is **native → free → premium** (A–Z within each group). Selector labels end with the archive version in parentheses (e.g. `(1.24.1)`). A permanent status line under the control reports that all archives are valid, or how many were rejected, with **Show more** / **Show less** to list rejected filenames. Choosing a listed ZIP runs the same client-side import as a manual ZIP import. The bundled `speech2texte` tree remains the default first-boot profile; Options does not delete that folder from the install.
