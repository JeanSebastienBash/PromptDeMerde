...
function pdm_resolve_llm_timeout_sec() {
    // 0 = illimité (UI) ; >0 = secondes ; max UI = 86400
    $default = 1000;
    $max = 86400;
    $raw = $_SERVER['HTTP_X_PDM_LLM_TIMEOUT_SEC'] ?? '';
    if ($raw === '') {
        return $default;
    }
    $sec = (int) $raw;
...
/** Timeout socket/stream PHP : 0 UI → durée pratique très longue (PHP 0 = immédiat / défaut) */
function pdm_stream_timeout_sec($uiSec) {
    $uiSec = (int) $uiSec;
    if ($uiSec <= 0) {
        return 86400 * 7;
    }
...
require_once dirname(__DIR__, 2) . '/api/lib.php';
$isProdEnv = pdm_resolve_deployment()['isProd'];
...
header('Access-Control-Allow-Headers: Content-Type, X-Ollama-Url, Authorization, X-PDM-Proxy-Token, X-PDM-Llm-Timeout-Sec');
...
$pdm_llm_timeout_sec = pdm_resolve_llm_timeout_sec();
$pdm_stream_timeout_sec = pdm_stream_timeout_sec($pdm_llm_timeout_sec);
if ($pdm_llm_timeout_sec <= 0) {
    @set_time_limit(0);
} else {
    @set_time_limit($pdm_llm_timeout_sec + 120);
}
...
if ($method === 'POST') {
    ...
    if ($body) {
        // UTF-8 : si le corps contient des octets invalides, json_decode échoue
        // et on perdait le flag stream → buffer non-stream (hang puis dump).
        $tmp = json_decode($body, true);
        if (is_array($tmp) && isset($tmp['stream']) && $tmp['stream'] === true) {
            $isStream = true;
        } elseif (preg_match('/"stream"\s*:\s*true\b/', $body)) {
            $isStream = true;
        }
    }
}
...
if ($isStream && $method === 'POST') {
    ...
    header('X-PDM-Stream: 1');
    // Apache : éviter tout buffer résiduel
    ...
    stream_set_timeout($fp, $pdm_stream_timeout_sec);
    ...
    /**
     * Pass-through NDJSON Ollama (ne pas ré-encoder).
     * Ré-encoder ligne par ligne cassait/retardait le stream (UTF-8, json_encode false,
     * sensation « vide longtemps puis dump partiel » côté navigateur).
     */
...
$streamOpts = [
    'http' => [
        'method'          => $method,
        'timeout'         => pdm_stream_timeout_sec(pdm_resolve_llm_timeout_sec()),
        'ignore_errors'   => true,
...
if ($method === 'POST' && $body) {
    $streamOpts['http']['header'] .= "Content-Type: application/json; charset=utf-8\r\nContent-Length: " . strlen($body) . "\r\n";
