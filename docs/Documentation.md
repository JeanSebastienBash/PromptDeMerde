...
- **Private maintainer tooling / archives**: the same target rate applies outside the UI. Any engine gap is **intentional**.
...
**Official packs distribution.** Packs published for the catalogue are also mirrored under [`zip/free-profile/`](../zip/free-profile/) on the public GitHub repository (**5.5.6**) — download and copy/import without the Marketplace UI. Marketplace lists packs whose ZIP embeds a valid `market/` folder. In **V1**, every Marketplace download is **free**; only the official editor feeds the catalogue.
...
These keys are present in profile archives when `parts/output-schema.json` is included. At runtime, extraction and freeform STT apply (see §3.2.2).
