 ...
 *
 * Synopsis : Transcription d'un fichier audio ou vidéo via Whisper Maxi (même moteur que la dictée).
 * Objectif : decode (piste audio) → ensureModel(whisper-maxi) → transcribeBuffer par chunk → texte joint.
 */
...
export function syncSttUiAfterFileImport() {
    ...
    var processing = !!(window.PDM.App && window.PDM.App.isWorkspaceAudioProcessing &&
        window.PDM.App.isWorkspaceAudioProcessing());
    if (els) {
