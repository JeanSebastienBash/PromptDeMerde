...
Shared.DICTATION_WARN_BEEP_MS = 1500;
...
Shared.playDictationBeep = function(kind) {
    ...
    function play() {
        ...
        if (kind === 'start') {
            ...
            for (i = 0; i < 3; i++) {
                var t0 = now + i * 0.48;
                Shared._playTone(ctx, 523, t0, 0.14, 0.38);
                Shared._playTone(ctx, 659, t0 + 0.18, 0.14, 0.38);
            }
        }
    }
    if (ctx.state === 'suspended' && ctx.resume) {
...
Shared.playDictationWarnBeep = function() {
    Shared.playDictationBeep('warn');
    return Shared.DICTATION_WARN_BEEP_MS || 1500;
};

Shared.waitDictationWarnBeep = function() {
    var ms = Shared.playDictationWarnBeep() || 1500;
