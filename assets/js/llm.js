 ...
 *
 * Pilote (norme snap 42) : dès qu’un format JSON/schema est présent → think:false.
 * Gemma/Ollama : réflexion et grammar JSON ne se mélangent pas dans la même requête.
 */
(function() {
...
function buildDefaultSystemPrompt() {
    ...
    if (CS && typeof CS.getLocaleDefault === 'function') {
        return CS.getLocaleDefault('DEFAULT_SYSTEM_PROMPT');
    }
...
function getContextInjectHeader() {
    ...
    var CS = window.PDM && window.PDM.ConfigSchema;
    if (CS && typeof CS.getLocaleDefault === 'function') {
        return CS.getLocaleDefault('DEFAULT_CONTEXT_INJECT_HEADER');
    }
...
function mergeLlmOptions(options, explicitModelId) {
    ...
    if (window.PDM.Storage && window.PDM.Storage.getLlmTemperature) {
        var temp = window.PDM.Storage.getLlmTemperature();
        // 0 = déterministe (STT) — envoyer ; évite le défaut Modelfile non-zéro
        if (Number.isFinite(temp)) merged.temperature = temp;
    }
    if (window.PDM.Storage && window.PDM.Storage.getLlmMaxTokens) {
        var maxTok = window.PDM.Storage.getLlmMaxTokens();
        // UI 0 = illimité → maxTokens:-1 (ollama.js mappe vers un plafond haut, pas -1 Ollama fragile)
        // Safe AVEC format car on force think:false juste après (norme snap 42).
        merged.maxTokens = maxTok > 0 ? maxTok : -1;
    }
    // NORME SNAP 42 / Gemma+Ollama : format JSON ⇒ think:false (pas think+format)
...
window.PDM.LLM = {
    ...
    sniperise: function(provider, model, token, systemPrompt, profiles, userPrompt, options) {
        ...
        if (!sys || !String(sys).trim()) {
            ...
            var noCtxMsg = I ? I.t('js.errors.noContext')
                : 'Aucune instruction pour le modèle : active le prompt système ou un prompt de contexte.';
            return Promise.reject(new Error(noCtxMsg));
        }
