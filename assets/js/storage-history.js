...
S.clearAll = function() {
    S.wipeAllUserData({ preserveCustomProfiles: false });
    if (typeof S.wipeBrowserCaches === 'function') {
        return S.wipeBrowserCaches();
    }
    var AB = window.PDM && window.PDM.StorageAudioBlobs;
    if (AB && AB.clearAll) return AB.clearAll();
...
S.normalizeCleanEntry = function(entry) {
    ...
    if (entry.audioSegmentCount != null) item.audioSegmentCount = entry.audioSegmentCount;
    if (entry.trace && typeof entry.trace === 'object') {
        try {
            item.trace = JSON.parse(JSON.stringify(entry.trace));
        } catch (e) {
