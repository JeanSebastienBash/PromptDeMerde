...
/**
 * Gather relevant audioRefs from workspace and history
 */
...
/**
 * Export configuration with audio blobs (asynchronous version)
 */
S.exportConfigAsync = function(options) {
    options = options || {};
    var includeAudio = options.includeAudio !== false; // Par défaut true
    var maxBlobSize = options.maxBlobSize || (50 * 1024 * 1024); // 50MB par défaut
...
/**
 * Import audio blobs from configuration (asynchronous version)
 */
...
/**
 * Import configuration with audio blobs (asynchronous version)
 */
S.importConfigAsync = function(data, options) {
    options = options || {};
    var includeAudio = options.includeAudio !== false;
    
    // Import configuration normale d'abord
    var result = S.importConfig(data, options);
    if (!result.ok) {
        return Promise.resolve(result);
    }
    
    // Si pas d'audio ou module indisponible, retourner immédiatement
    ...
    return S._importAudioBlobs(data.pdm_audio_blobs)
        ...
        .catch(function(error) {
            ...
            return {
                ok: true, // Config principale importée avec succès
                format: result.format,
                audioImported: false,
