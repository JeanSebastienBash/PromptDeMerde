...
Shared.isLinux = function() {
    var ua = navigator.userAgent || '';
    var platform = navigator.platform || '';
    return /Linux|X11/.test(ua) || /Linux/i.test(platform);
};

Shared.getLanHelpText = function() {
    if (Shared.isChromium()) return Shared.sttT('help.lanChromium');
    if (Shared.isFirefox()) return Shared.sttT('help.lanFirefox');
    return Shared.sttT('help.lanGeneric');
};
...
Shared.refreshStripHint = function() {
    ...
    if (Shared._stripHints.custom) {
        ...
        msg = Shared.sttT('status.lanStrip');
        kind = 'error';
...
Shared.notifyStt = function(msg, type) {
    ...
    if (Shared.isWorkspaceView() && (type === 'err' || type === 'error' || type === 'warn' || type === 'info')) {
        var engage = document.getElementById('stt-engage-msg');
        if (engage) {
            engage.textContent = msg;
            engage.hidden = false;
