...
function setSttBadgeContent(badge, icon, label) {
    if (!badge) return;
    var text = label || '';
    if (icon && text) {
        var safeLabel = String(text)
            .replace(/&/g, '&amp;')
            .replace(/</g, '&lt;')
            .replace(/>/g, '&gt;')
            .replace(/\n/g, '<br>');
        badge.innerHTML = '<span class="stt-badge-icon" aria-hidden="true">' + icon + '</span>' +
            '<span class="stt-badge-label">' + safeLabel + '</span>';
    } else {
...
Shared.setState = function(els, next) {
    ...
    var facade = window.PDM && window.PDM.STT;
    if (facade && facade.syncEngageMsgFromState) facade.syncEngageMsgFromState();
};
...
Shared.setStatus = function(els, text, kind) {
    if (!els.status) {
        ...
        for (var j = 0; j < micBadges.length; j++) {
            ...
            if (kind === 'listening') micBadges[j].textContent = Shared.sttT('badge.active');
        }
        return;
    }
...
Shared.updateButton = function(els, active, mode) {
    ...
    if (active && mode === 'loading') {
        els.btn.textContent = Shared.getBtnCancelLoad();
    } else {
        els.btn.textContent = active ? Shared.getBtnStop() : Shared.getBtnStart();
    }
...
Shared.setLoadProgress = function(els, opts) {
    ...
    var label = opts.label || Shared.sttT('loadModelDefault');
    ...
    if (window.PDM && window.PDM.STT && window.PDM.STT.reportLoadProgress && opts.syncReport !== false) {
        window.PDM.STT.reportLoadProgress({
            label: opts.label || Shared.sttT('loadModelDefault'),
...
Shared.updateModelStatusBar = function(root, state) {
    ...
    var shortLabel = state.badge || '';
    var line = state.line || shortLabel;
    var phase = state.phase || 'pending';
    if (badge) {
        var label = shortLabel || line;
        if (phase === 'loading' && state.percent != null && state.percent >= 0) {
            label = Math.round(state.percent) + ' %';
            setSttBadgeContent(badge, MODEL_BADGE_ICONS.loading, label);
        } else if (phase === 'ready' || (label && label.indexOf('(') >= 0)) {
            badge.textContent = label;
        } else {
            setSttBadgeContent(badge, MODEL_BADGE_ICONS[phase] || MODEL_BADGE_ICONS.pending, label);
        }
        badge.className = 'stt-model-badge stt-model-badge-' + phase;
        badge.title = line;
...
Shared.updateMicStatusBar = function(root, mic) {
    ...
    var shortLabel = mic.badge || '';
    var line = mic.line || shortLabel;
    var phase = mic.phase || 'unknown';
    var hideBadge = false;
    if (badge) {
        if (hideBadge) {
            badge.hidden = true;
        } else {
            var icon = MIC_BADGE_ICONS[phase] || MIC_BADGE_ICONS.unknown;
            setSttBadgeContent(badge, icon, shortLabel || line);
            badge.className = 'stt-mic-badge stt-mic-badge-' + phase;
            badge.title = line;
            badge.setAttribute('aria-label', line);
            badge.hidden = false;
        }
...
Shared.startTickTimer = function(timers, getState, els, startTime) {
    ...
    timers.tickTimer = setInterval(function() {
        ...
        var base = Shared.sttT('listeningSpeakNow') || '\u00c9coute en cours\u2026';
        Shared.setStatus(els, base + ' ' + label, 'listening');
    }, 1000);
};
