...
var replayLiveFn = null;

export function setReplayLiveFn(fn) {
    replayLiveFn = fn;
}

export function replayLiveIfPending(id, els) {
    if (replayLiveFn) replayLiveFn(id, els);
}
...
function detectBackend(opts) {
    ...
    if (S.isWhisperGpuBroken && S.isWhisperGpuBroken()) {
        var caps = S.getGpuCaps();
        if (caps.shaderF16) return Promise.resolve('wasm');
    }
...
export function whisperRuntimeWasmFallback(id, els, reason) {
    ...
    var notifMsg = (reason === 'validation-error' || reason === 'gpu-broken' || reason === 'transcribe-error')
        ? T('gpuIncompatibleWhisper')
        : T('gpuFallbackWhisper');
    window.PDM.UI.notif(notifMsg, 'info');
    if (els && els.status && st.state === S.STATE_LISTENING) {
        S.setStatus(els, T('cpuFallbackInProgress'), 'listening');
    }
    st.modelLoadGen++;
    ...
    return terminateWorker(st).then(function() {
        ...
        if (st.state === S.STATE_LISTENING && els && els.status) {
            S.setStatus(els, T('listeningCpu'), 'listening');
        }
        window.PDM.UI.notif(T('notif.whisperCpuOk'), 'ok');
        if (window.PDM && window.PDM.STT && window.PDM.STT.renderUi) window.PDM.STT.renderUi();
        replayLiveIfPending(id, els);
        return true;
    }).catch(function(err) {
        st._wasmFallbackActive = false;
...
export function checkModelFiles(id) {
    var base = modelDirFor(id);
    var configUrl = S.modelAbsoluteUrl(base + 'config.json');
    return fetch(configUrl, { method: 'HEAD', cache: 'no-store' }).then(function(r) {
        ...
        function headOk(rel) {
            return fetch(S.modelAbsoluteUrl(base + rel), { method: 'HEAD', cache: 'no-store' })
                .then(function(or) { return or.ok; })
                .catch(function() { return false; });
        }
        return Promise.all([
            Promise.all(encoders.map(headOk)),
            Promise.all(decoders.map(headOk))
        ]).then(function(pairs) {
            if (pairs[0].some(Boolean) && pairs[1].some(Boolean)) return true;
...
export function ensureModel(id, els, opts) {
    ...
    if (!background) {
        ...
        tracker.show();
        tracker.phase(T('loadEngineWorker'), 0, cfg.sizeLabel);
    } else {
        tracker.phase(T('loadEngineWorker'), 0, cfg.sizeLabel);
    }
    ...
    st.modelPromise = checkModelFiles(id).then(function() {
        ...
        try {
            ...
            st.libLoadDetail = e && e.message ? e.message : 'Worker unavailable';
        ...
        tracker.phase(T('loadLibrary'), 5, backend.toUpperCase());
        return new Promise(function(resolve, reject) {
            ...
            worker.postMessage({
                ...
                backend: backend,
                wantedGpu: S.wantsGpuCompute(),
                gpuKnownBroken: !!(S.isWhisperGpuBroken && S.isWhisperGpuBroken()),
                lang: st.lang,
                paths: {
