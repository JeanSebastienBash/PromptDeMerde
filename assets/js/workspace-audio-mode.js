 ...
 *
 * Synopsis : Bascule mode fichier audio/vidéo du Workspace (statut, readonly temporaire).
 * Objectif : Verrouiller la dictée seulement pendant le traitement ; après transcription,
 *            réouvrir dictée / édition sans exiger Reset, en gardant la provenance fichier.
 */
(function(){
"use strict";
...
function hasAudioFileProvenance() {
    try {
        var a = window.PDM.Storage.getWorkspaceAudio && window.PDM.Storage.getWorkspaceAudio();
        return !!(a && a.inputSource === 'audio-file');
    } catch (e) {
        return false;
    }
}
...
A.hasWorkspaceAudioFileProvenance = function() {
    return hasAudioFileProvenance();
};
...
function refreshSttAfterAudioUi() {
    if (window.PDM && window.PDM.STT && window.PDM.STT.syncSttAfterFileImport) {
...
A.setWorkspaceAudioMode = function(on, fileName) {
    ...
    if (!on) {
        resetStatusEl(st);
        if (window.PDM && window.PDM.WorkspaceDictationAudio && window.PDM.WorkspaceDictationAudio.resetHybridState) {
...
A.setWorkspaceAudioDone = function(fileName, onNewInput) {
    ...
    var f = field();
    if (f) f.classList.remove('ws-audio-mode');
