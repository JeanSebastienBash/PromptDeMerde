...
function createWhisperEngine(engineId) {
    ...
    var engine = {
        ...
        getHelpText: function() {
            ...
            var gpu = S.hasWebGPU() ? T('helpWhisperGpu') : T('helpWhisperCpu');
            return T('help.whisper', { name: cfg.shortName, size: cfg.helpSize, backend: gpu, seconds: SEGMENT_SECONDS });
        },
        ...
        toggle: function(c) {
            ...
            c.facade.ensureMicAccess({ fromToggle: true }).then(function(s) {
                ...
                if (micErr) {
                    ...
                    var base = libLoad ? T('errorWhisperLib')
                        : (missing ? T('errorWhisperMissing')
                        : (oom ? T('errorWhisperOom') : T('errorWhisperFailed')));
                    var detail = libLoad ? st.libLoadDetail : (err && err.message ? err.message : '');
                    S.setStatus(els, base + ' ' + detail, 'error');
                    var hint = libLoad ? T('errorWhisperLibCause', { detail: st.libLoadDetail || '' })
                        : (missing ? modelLoadErrorHint(engineId)
                        : (oom ? T('errorCloseTabsVosk') : modelLoadErrorHint(engineId)));
        ...
        stop: function(c, opts) {
            ...
            if (!opts.silent) {
                S.setStatus(els, wasListening ? T('dictationDone') : T('dictationCancelled'), 'ok');
            }

            teardownStream(engineId);

            if (wasListening && !producedText && !st.gotAnyText && !st.chunkErrorReported && !opts.silent) {
                window.PDM.UI.notif(T('notifNoTranscriptWhisper', { name: cfg.shortName }), 'info');
            }
        ...
        getStatusLabel: function() {
            var st = getPool(engineId);
            if (st.loadedModelId) {
                return T('statusModelReady', {
                    name: cfg.shortName,
                    backend: S.formatBackendLoadedLabel(st.usedBackend, st.usedDtype)
                });
            }
            if (engine.isModelLoading()) return T('statusBackgroundLoading', { name: cfg.shortName, backend: S.formatBackendLoadingLabel(engineId, S.wantsGpuCompute()) });
        ...
        getLoadedDtypeKey: function() {
            var st = getPool(engineId);
            return st.workerReady ? (st.usedDtype || null) : null;
        },
    };
